...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sentence in parallel_corpus:
            scr_idx, target_idx = sentence.source_tokens, sentence.target_tokens
            posterior = self.translation_probs[np.ix_(scr_idx, target_idx)]
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for k, sentence in enumerate(parallel_corpus):
            src_idx, target_idx = sentence.source_tokens, sentence.target_tokens
            elbo += np.sum(posteriors[k] * np.log(self.translation_probs[np.ix_(src_idx, target_idx)] / len(posteriors[k][:,0]) + 1e-20) - posteriors[k] * np.log(posteriors[k] +1e-20))
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for k, sentence in enumerate(parallel_corpus):
            scr_idx, target_idx = sentence.source_tokens, sentence.target_tokens
            np.add.at(self.translation_probs, np.ix_(scr_idx, target_idx), posteriors[k])

        divider = self.translation_probs.sum(axis=1)
        self.translation_probs = self.translation_probs.T
        self.translation_probs /= divider
        self.translation_probs = self.translation_probs.T
    ...
    def align(self, sentences):
        ...
        for sentence in sentences:
            alignment = []
            for target_idx in range(len(sentence.target_tokens)):
                scr_idx = self.translation_probs[list(sentence.source_tokens), sentence.target_tokens[target_idx]].argmax()
                answer = scr_idx + 1, target_idx + 1
...
class WordPositionAligner(WordAligner):
    ...
    def _get_probs_for_lengths(self, src_length: int, tgt_length: int):
        ...
        """
        if (src_length, tgt_length) in self.alignment_probs.keys():
    ...
    def _e_step(self, parallel_corpus):
        ...
        for sentence in parallel_corpus:
            scr_idx, target_idx = sentence.source_tokens, sentence.target_tokens
            prior = self._get_probs_for_lengths(len(scr_idx), len(target_idx))
            posterior = prior * self.translation_probs[np.ix_(scr_idx, target_idx)]
            posterior /= posterior.sum(axis=0)
            posteriors.append(posterior)
    ...
    def _compute_elbo(self, parallel_corpus, posteriors):
        ...
        for k, sentence in enumerate(parallel_corpus):
            src_idx, target_idx = sentence.source_tokens, sentence.target_tokens
            part_1 = xlogy(posteriors[k], self.translation_probs[np.ix_(src_idx, target_idx)])
            part_2 = xlogy(posteriors[k], self._get_probs_for_lengths(len(src_idx), len(target_idx)))
            part_3 = xlogy(posteriors[k], posteriors[k])
            elbo += np.sum(part_1 + part_2 - part_3)
    ...
    def _m_step(self, parallel_corpus, posteriors):
        ...
        for key in self.alignment_probs.keys():
            self.alignment_probs[key] = np.zeros_like(self.alignment_probs[key])

        #count = {}

        for k, sentence in enumerate(parallel_corpus):
            scr_idx, target_idx = sentence.source_tokens, sentence.target_tokens
            self.alignment_probs[(len(scr_idx), len(target_idx))] = self._get_probs_for_lengths(len(scr_idx), len(target_idx)) + posteriors[k]
            #count[(len(scr_idx), len(target_idx))] = count.get((len(scr_idx), len(target_idx)), 0) + 1

        for key in self.alignment_probs.keys():
            self.alignment_probs[key] /= self.alignment_probs[key].sum(axis=0) #self.alignment_probs.get(key) / count[key]
